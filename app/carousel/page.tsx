  ...
  if (candidates.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center"
        ...
        <p className="text-[12px]">YouTube 큐에서 영상을 예약 확정하면 여기서 캐러셀을 생성할 수 있습니다</p>
      </div>
    );
  }

  return <CarouselGenerator candidates={candidates} />;
}
...
export default function CarouselPage() {
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-[18px] font-bold" style={{ color: "var(--text)" }}>캐러셀 생성</h1>
        <p className="text-[12px] mt-1" style={{ color: "var(--text-muted)" }}>
          예약 확정 영상을 선택하고 Claude AI로 인스타그램 캐러셀 슬라이드 문구를 자동 생성합니다
        </p>
      </div>
      <Suspense
        fallback={
