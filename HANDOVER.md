...
캐러셀은 특정 YouTube 영상과 무관한 독립적인 인스타그램 콘텐츠 제작 도구.
"이 주제에 어울리는 노래 N곡" 형식의 테마형 곡 소개 캐러셀을 생성.

**플로우:**
1. 사용자가 주제 입력 또는 추천 주제 선택
   - 추천 주제 예시: "비 오는 날 듣기 좋은 노래", "드라이브할 때 듣는 노래", "새벽에 혼자 듣는 노래", "실연 후 들으면 좋은 노래", "90년대 감성 팝" 등
   - 직접 입력도 가능
2. 슬라이드 장 수 선택 (사용자가 직접 선택, 예: 3~10장)
3. Claude API가 주제에 맞는 곡 목록 + 각 슬라이드 문구 생성
4. 각 슬라이드에 이미지 첨부:
   - **AI 이미지 생성**: 슬라이드 분위기에 맞는 이미지 프롬프트를 Claude가 생성 → OpenAI DALL-E 3 API 또는 외부 이미지 생성 API 호출
   - **직접 업로드**: 사용자가 각 슬라이드에 이미지 파일 직접 업로드 가능
5. 전체 캡션 + 해시태그 자동 생성
6. 결과물 복사 버튼 포함
...
- 곡 제목 + 아티스트명
- 감성 키워드 2~3개
- 한 줄 소개 문구 (채널 감성에 맞게: 새벽, 그리움, 감성적)
- 이미지 (AI 생성 또는 업로드)

**마지막 슬라이드 (고정):**
- CTA 문구: "더 많은 감성 음악은 @dawn.town.music 에서" 형식
- 채널 소개 + 팔로우 유도

**이미지 생성 API 결정 필요:**
- OpenAI DALL-E 3 추천 (`OPENAI_API_KEY` 환경변수 추가 필요)
- 또는 이미지 생성 없이 프롬프트만 출력하고 사용자가 외부 툴 사용하는 방식도 가능
- 우선 구현 시: 이미지 없이 텍스트 결과물만 생성하고, 이미지는 사용자 업로드로 처리 추천
